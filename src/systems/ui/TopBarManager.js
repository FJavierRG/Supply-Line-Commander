    ...
    renderCurrency(ctx) {
        ...
        if (sprite) {
            ...
            ctx.drawImage(sprite, 
                currencyIcon.x + offsetX + shakeOffsetX, 
        ...
        const textX = currencyIcon.x + currencyIcon.w + 5 + shakeOffsetX;
        const textY = currencyIcon.y + currencyIcon.h / 2 - 10; // Mover un poco arriba
        
        // 🆕 MODIFICADO: Rojo si está vibrando (no puede pagar), si es negativo (deuda), o dorado normal
        ctx.fillStyle = isShaking ? '#ff4444' : (currency < 0 ? '#ff4444' : '#ffd700');
        ...
        const incomeTextX = textX; // Ya incluye shakeOffsetX
        const incomeTextY = currencyIcon.y + currencyIcon.h / 2 + 12; // Mover un poco abajo
        
        ctx.fillStyle = isShaking ? '#ff6666' : '#4ecf75'; // Rojo claro si shake, verde si normal
